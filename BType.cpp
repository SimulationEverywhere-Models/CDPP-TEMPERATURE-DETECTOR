...
********************************************************************/
Model &BType::externalFunction( const ExternalMessage &msg )
{
	if(msg.port()==in )
	{
		A+=static_cast<float>(msg.value());
		N++;
...
********************************************************************/
Model &BType::internalFunction( const InternalMessage & )
{
	this->passivate();
...
********************************************************************/
Model &BType::outputFunction( const InternalMessage &msg )
{
	if(iState)
		{
			B=(A/N)/10;
			sendOutput(msg.time(), out, B);
			N=0;
			B=0;
			A=0;
		}
